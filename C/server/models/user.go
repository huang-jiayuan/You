...
type User struct {
	...
	VipExpireTime time.Time `gorm:"column:vip_expire_time;type:datetime(3);comment:会员过期时间;" json:"vip_expire_time"`        // 会员过期时间
	Balance       float32   `gorm:"column:balance;type:float;comment:虚拟币余额;" json:"balance"`                               // 虚拟币余额
	...
	FreezeEndTime time.Time `gorm:"column:freeze_end_time;type:datetime(3);comment:冻结/封禁结束时间;" json:"freeze_end_time"`     // 冻结/封禁结束时间
