...
type User struct {
	...
	IsAuth        uint32    `gorm:"column:is_auth;type:int(1) UNSIGNED ZEROFILL;comment:实名认证状态（0：未认证1：已认证2）;not null;default:0;" json:"is_auth"` // 实名认证状态（0：未认证1：已认证2）
	Mute          string    `gorm:"column:mute;type:varchar(1);comment:是否禁言;not null;default:0;" json:"mute"`                                            // 是否禁言
}
