...
// 用户表
type User struct {
	Id            uint64    `gorm:"column:id;type:bigint UNSIGNED;comment:用户唯一ID;primaryKey;not null;" json:"id"`                                          // 用户唯一ID
	Username      string    `gorm:"column:username;type:varchar(32);comment:登录账号(手机号/第三方账号);not null;" json:"username"`                                    // 登录账号(手机号/第三方账号)
	Password      string    `gorm:"column:password;type:varchar(64);comment:登录密码(SHA-256加密);default:NULL;" json:"password"`                                // 登录密码(SHA-256加密)
	...
	VoiceTag      string    `gorm:"column:voice_tag;type:varchar(100);comment:声音标签(逗号分隔);default:NULL;" json:"voice_tag"`                                  // 声音标签(逗号分隔)
	InterestTags  string    `gorm:"column:interest_tags;type:varchar(200);comment:兴趣标签(逗号分隔);default:NULL;" json:"interest_tags"`                          // 兴趣标签(逗号分隔)
	...
	VipExpireTime time.Time `gorm:"column:vip_expire_time;type:datetime;comment:会员过期时间;default:NULL;" json:"vip_expire_time"`                              // 会员过期时间
	...
	FreezeReason  string    `gorm:"column:freeze_reason;type:varchar(200);comment:冻结/封禁原因;default:NULL;" json:"freeze_reason"`                             // 冻结/封禁原因
	FreezeEndTime time.Time `gorm:"column:freeze_end_time;type:datetime;comment:冻结/封禁结束时间;default:NULL;" json:"freeze_end_time"`                           // 冻结/封禁结束时间
	IsAuth        uint32    `gorm:"column:is_auth;type:int(10) UNSIGNED ZEROFILL;comment:实名认证状态（0：未认证1：已认证2）;not null;default:0000000000;" json:"is_auth"` // 实名认证状态（0：未认证1：已认证2）
	Phone         string    `gorm:"column:phone;type:char(11);comment:手机号;default:NULL;" json:"phone"`                                                     // 手机号
}
